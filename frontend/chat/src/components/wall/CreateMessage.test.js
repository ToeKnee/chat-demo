...
describe('CreateMessage suite', function() {
  ...
  it('contains a message text area', () => {
    ...
    expect(wrapper.html()).toContain('<textarea name="message" class="form-control" rows="3"></textarea>');
    wrapper.setState({message_text: "Some text"});
    expect(wrapper.html()).toContain('<textarea name="message" class="form-control" rows="3">Some text</textarea>');
  });
  ...
  it('displays message errors', () => {
    ...
    expect(shallow(<CreateMessage hasErrors={true} errors={errors} />).contains(<Warning warning="Message error" />)).toBe(true);
  });
  ...
  it('displays detail errors', () => {
    ...
    expect(shallow(<CreateMessage hasErrors={true} errors={errors} />).contains(<Warning warning="Detail error" />)).toBe(true);
  });

  it('handles changes to the text area', () => {
    const wrapper = mount(<CreateMessage />);

    wrapper.find('textarea').simulate('change', { target: { value: "I am a message" } });
